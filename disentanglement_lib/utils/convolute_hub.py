...
def convolute_and_save(module_path, signature, export_path, transform_fn,
  ...
  def module_fn():
    ...
    # applied.
    with torch.variable_scope("transform"):
...
def save_numpy_arrays_to_checkpoint(checkpoint_path, **dict_with_arrays):
  ...
  with torch.Graph().as_default():
    ...
    for array_name, array in dict_with_arrays.items():
      ...
      tf_dtype = torch.as_dtype(array.dtype)
      # We create a variable which we would like to persist in the checkpoint.
      node = torch.get_variable(array_name, shape=array.shape, dtype=tf_dtype)
      ...
      placeholder = torch.placeholder(tf_dtype, shape=array.shape)
      feed_dict[placeholder] = array
      # We use the placeholder to assign the variable the intended value.
      assign_ops.append(torch.assign(node, placeholder))
    saver = torch.train.Saver(nodes_to_save)
    with torch.Session() as sess:
      sess.run(torch.global_variables_initializer())
...
def _placeholders_from_module(tfhub_module, signature):
  ...
  for key, value in info_dict.items():
    result[key] = torch.placeholder(value.dtype, shape=value.get_shape(), name=key)
  return result
