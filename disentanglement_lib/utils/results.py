...
def update_result_directory(result_directory,
    ...
    if old_result_directory is not None:
        ...
        if not os.path.isdir(result_directory):
            pathlib.Path(result_directory).mkdir(parents=True)
    ...
    results_json_path = os.path.join(json_dir, "{}_results.json".format(step_name))
    ...
    def default(self, obj):
        if isinstance(obj, (np.float_, np.float32, np.float16, np.float64)):
            ...
            if isinstance(obj, torch.Tensor):
                return self.default(obj.cpu().detach().numpy())
        # return 个锤子，直接报错不好么？
