...
class Evaluate:
	...
	@torch.no_grad()
	def compute_H_xCz(model, device='cpu', num_samples=10000):
		...
		for i in range(model.num_latent + 1):
			if i == 0:
				...
				z1 = torch.randn(test_samples, 1, i, device=device).repeat([1, test_samples, 1]).view(num_samples, -1)
				z2 = torch.randn(1, test_samples, model.num_latent - i, device=device).repeat(
					[test_samples, 1, 1]).view(num_samples, -1)
				z = torch.cat([z1, z2], 1)
				recons = torch.sigmoid(decoder(z))
				img_size = np.prod(recons.shape[1:])
	...
	def evaluate(self, model) -> None:
		model.eval()
		dic_log = (self.compute_recons(model, 'cuda'))
