...
def estimate_JEMMIG_cupy(z_mean, z_stddev, y, x_weights=None, num_samples=10000,
    ...
    with cp.cuda.Device(gpu):
        ...
        H_z_yk = cp.asarray(np.zeros([y_dim], dtype=np.float32))
        ...
        while count < num_samples:
            ...
            for k in range(y_dim):
                ...
                t = (cp.expand_dims(cp.expand_dims(x_weights, axis=-1), axis=-1) *
                     q_zi_cond_x_all_batch * q_yk_cond_x_all_batch)
                q_zi_yk_batch = cp.sum(t, axis=0)

                q_z_yk_batch = cp.sum(cp.prod(t, axis=2), axis=0)

                # Sum (S_batch, z_dim) over S_batch => (z_dim, )
                H_zi_yk[:, k] += -cp.sum(cp.log(cp.maximum(q_zi_yk_batch, eps)), axis=0)
                # Sum (S_batch) over S_batch => (1, )
                H_z_yk[k] += -cp.sum(cp.log(cp.maximum(q_z_yk_batch, eps)), axis=0)
